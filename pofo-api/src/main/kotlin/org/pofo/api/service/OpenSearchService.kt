...
class OpenSearchService(
    ...
    fun indexTechStack(techStack: TechStackAutoComplete): Boolean {
        ...
        return try {
            ...
            val response: IndexResponse = openSearchClient.index(request)
            response.result().name == "created"
        } catch (e: Exception) {
    ...
    fun searchSingleField(
        ...
        try {
            ...
            val request =
                SearchRequest.of { searchRequest ->
                    searchRequest
                        .index(indexName)
                        .query { query ->
                            query.match { matchQuery ->
                                matchQuery.field(fieldName)
                                matchQuery.query(FieldValue.of(keyword))
                                matchQuery
                            }
                        }
                }

            val response: SearchResponse<TechStackAutoComplete> =
                openSearchClient.search(request, TechStackAutoComplete::class.java)
